...
def main():
	...
	parser.add_argument('--use_route',
											type=int,
											default=int(config.use_route),
											help='Whether to use route prediction as auxiliary loss for training(from PlanT).')
	parser.add_argument('--use_hdmap',
											type=int,
											default=int(config.use_hdmap),
											help='Whether to use hdmap prediction as auxiliary loss for training.')
	parser.add_argument('--use_vehicle_occupancy',
											type=int,
											default=int(config.use_vehicle_occupancy),
											help='Whether to use vehicle_occupancy prediction as auxiliary loss for training.')
	parser.add_argument('--use_pedestrian_occupancy',
											type=int,
											default=int(config.use_pedestrian_occupancy),
											help='Whether to use pedestrian_occupancy prediction as auxiliary loss for training.')
	...
	if config.use_plant:
		...
		config.detailed_loss_weights['loss_hdmap'] = 0.0 # tf+
	...
	if not config.use_depth:
		config.detailed_loss_weights['loss_depth'] = 0.0
	#tf+
	if not config.use_hdmap:
		config.detailed_loss_weights['loss_hdmap'] = 0.0
	...
	if not config.use_vehicle_occupancy:
		config.detailed_loss_weights['loss_vehicle_occupancy'] = 0.0
	if not config.use_pedestrian_occupancy:
		config.detailed_loss_weights['loss_pedestrian_occupancy'] = 0.0	
	...
	if config.freeze_backbone:
		...
		config.detailed_loss_weights['loss_depth'] = 0.0
		config.detailed_loss_weights['loss_hdmap'] = 0.0
		config.detailed_loss_weights['loss_route'] = 0.0  
		config.detailed_loss_weights['loss_center_heatmap'] = 0.0
		config.detailed_loss_weights['loss_wh'] = 0.0
		config.detailed_loss_weights['loss_offset'] = 0.0
		config.detailed_loss_weights['loss_yaw_class'] = 0.0
	...
	if config.freeze_backbone:
		...
		if config.use_hdmap:
			model.hdmap_decoder.requires_grad_(False)

		if config.use_route:
			model.route_decoder.requires_grad_(False)
   
		if config.use_vehicle_occupancy:
			model.vehicle_occupancy_decoder.requires_grad_(False)

		if config.use_pedestrian_occupancy:
			model.pedestrian_occupancy_decoder.requires_grad_(False)
	...
	def load_data_compute_loss(self, data, validation=False):
		...
		if self.config.use_plant:
			...
			if self.config.use_route:
				route_label = data['route'][:, :self.config.predict_checkpoint_len].to(self.device, dtype=torch.float32)
			else:
				route_label = None
			
			if self.config.use_hdmap:
			# tf+
				hdmap_label = data['hdmap'].to(self.device, dtype=torch.long)
			else:
				hdmap_label = None
			...
			if self.config.use_vehicle_occupancy:
				vehicle_occupancy_label = data['vehicle_occupancy'].to(self.device, dtype=torch.long)
				#print(vehicle_occupancy_label.shape)
			else:
				vehicle_occupancy_label = None
			if self.config.use_pedestrian_occupancy:
				pedestrian_occupancy_label = data['pedestrian_occupancy'].to(self.device, dtype=torch.long)
				#print(pedestrian_occupancy_label.shape)
			else:
				pedestrian_occupancy_label = None
			pred_wp,\
			pred_target_speed,\
			pred_checkpoint,\
			pred_semantic, \
		...
		if self.config.use_plant:
			...
			losses = compute_loss(pred_wp=pred_wp,
														...
														pred_hdmap=pred_hdmap,
														pred_route=pred_route,
														pred_vehicle_occupancy=pred_vehicle_occupancy,
														pred_pedestrian_occupancy=pred_pedestrian_occupancy,
														...
														hdmap_label=hdmap_label,
														route_label=route_label,
														vehicle_occupancy_label=vehicle_occupancy_label,
														pedestrian_occupancy_label=pedestrian_occupancy_label,
